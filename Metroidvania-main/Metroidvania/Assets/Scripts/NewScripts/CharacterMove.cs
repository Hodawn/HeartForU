...
public class CharacterMove : MonoBehaviour
{
    ...
    [Header("Shield Settings")] // 방패 관련 설정 추가
    [SerializeField] private GameObject shieldPrefab; // 방패 오브젝트 프리팹
    [SerializeField] private Transform shieldPosition; // 방패가 활성화될 위치
    [SerializeField] private float shieldDuration = 2.0f; // 방패 지속 시간
    private GameObject activeShield; // 현재 활성화된 방패
    private bool isShieldActive = false; // 방패가 활성화 상태인지 여부
    private float shieldCooldown = 5.0f; // 방패 재사용 대기시간
    private float shieldCooldownTimer = 0.0f; // 재사용 대기시간 타이머
    ...
    private void FixedUpdate()                      //FiexedUpdate에서 이동 구현을 한다. 
    {
        ...
        HandleShieldCooldown(); //방패 쿨다운
    }

    public void Move(float move, bool jump, bool dash, bool shield)
    {
        if (status.canMove)          //클래스에서 이동이 가능하다고 하면
        {
            ...
            {
                ActivateShield();
                Debug.Log("Shield is active during movement.");
            }
    ...
    {
        if (!isShieldActive && shieldCooldownTimer <= 0.0f)
        {
            activeShield = Instantiate(shieldPrefab, shieldPosition.position, Quaternion.identity, shieldPosition);
            isShieldActive = true;
            animator.SetBool("IsShielding", true);
            StartCoroutine(DeactivateShieldAfterDelay());
            shieldCooldownTimer = shieldCooldown; // 쿨다운 시작
        }
    }
